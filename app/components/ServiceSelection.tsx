...
import { discountText, serviceGroups, type Identity, type Registration } from '@/lib/registration'
...
export function ServiceSelection({ user, registration, onBack, onReset }: Props) {
  ...
  async function submitSurvey() {
    ...
    try {
      const result = await api<{ code: string; issuedAt: string }>('survey.submit', {
        body: { service: selectedService, application: application.trim(), clarity, serviceRating },
      })
      setCode(result.code)
  ...
  return <section className="space-y-7">
    ...
    {step === 4 && <div className="space-y-5">
      <div className="rounded-2xl border border-emerald-400/30 bg-emerald-400/10 p-5"><p className="text-sm font-semibold text-emerald-200">Kit aprobado</p><p className="mt-2 text-lg font-bold text-slate-50">{discountText}</p><p className="mt-3 break-words text-xs text-slate-300">Tu ID único: <span className="font-mono text-cyan-300">{code}</span></p><p className="mt-3 text-xs leading-5 text-slate-400">Válido por 14 días naturales a partir de la fecha de emisión. No transferible. No acumulable con otros descuentos, becas o promociones. Descuento aplicable sobre el precio lista antes de IVA.</p></div>
