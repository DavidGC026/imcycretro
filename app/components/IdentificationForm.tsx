...
import type { Identity } from '@/lib/registration'

type FormValues = Identity
...
export function IdentificationForm({ initialValues, onSubmit }: Props) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setNotice('')
    setLoading(true)
    const identity = { name: values.name.trim(), company: values.company.trim(), email: values.email.trim().toLowerCase() }
    try {
      await api('registration.save', { body: identity })
      onSubmit(identity)
    } catch (error) {
  ...
  return (
    <form onSubmit={submit} noValidate className="space-y-5">
        ...
        {fields.map(({ key, label, placeholder, icon: Icon, type }) => (
          <div key={key} className="space-y-2">
            ...
            <div className="relative">
              <input id={key} name={key} type={type} required disabled={loading} autoComplete={key === 'company' ? 'organization' : key} maxLength={key === 'name' ? 160 : key === 'company' ? 200 : 254} value={values[key]} placeholder={placeholder} aria-invalid={Boolean(errors[key])} aria-describedby={errors[key] ? `${key}-error` : undefined} onChange={(e) => { setValues({ ...values, [key]: e.target.value }); setErrors({ ...errors, [key]: undefined }) }} className={`w-full rounded-xl border bg-slate-800/80 py-3.5 pl-4 pr-11 text-sm text-slate-100 outline-none transition placeholder:text-slate-400 focus:ring-2 focus:ring-cyan-400/70 ${errors[key] ? 'border-red-400/80 focus:ring-red-400/50' : 'border-slate-700/80 focus:border-cyan-400/70'}`} />
      ...
      <div className="flex flex-wrap items-center gap-4 border-t border-slate-700/70 pt-5">
        <button type="submit" disabled={loading} className="inline-flex items-center gap-2 rounded-xl bg-cyan-700 px-7 py-3.5 text-sm font-bold tracking-[0.18em] text-white transition hover:bg-cyan-800 focus:outline-none focus:ring-2 focus:ring-cyan-300 disabled:cursor-wait disabled:opacity-70">{loading && <Loader2 aria-hidden="true" className="h-4 w-4 animate-spin" />} {loading ? 'VALIDANDO' : 'INGRESAR'}</button>
